"""
vector.py — Local, provider-agnostic embedding (vector) memory.
...
call, anything with that shape — so this module stays a thin storage +
similarity layer, matching the rest of the family's "bring your own
everything" design.

Requires ``numpy`` (the ``autourgos-memory[vector]`` extra).
"""
...
class VectorMemory(RetrievalAugmentedMemory):
    ...
    :class:`~autourgos_memory.semantic.KeywordMemory`, swapping TF-IDF for
